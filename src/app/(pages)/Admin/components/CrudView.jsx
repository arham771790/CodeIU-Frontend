...
const CrudView = ({ title, data, columns, onAddItem ,handletask,handleEdit}) => {
    console.log(data)
                ...
                <tbody>
                    {data.map(item => (
                        <tr key={item.id} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                            ...
                            <td className="p-4"><div className="flex gap-2"><button className="text-blue-400 hover:text-blue-300"><Pencil className="w-4 h-4" onClick={()=>handleEdit(item)}/></button><button className="text-red-400 hover:text-red-300"><Trash2 className="w-4 h-4 " onClick={()=>handletask(item)}/></button></div></td>
                        </tr>
                    ))}
                </tbody>
            </table>
