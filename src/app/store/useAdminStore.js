...
export const useAdminStore = create((set) => ({
  ...
  fetchAllUsers: async () => {
    try {
      ...
      const res = await axiosInstanceAuthService.get("/admin/getallusers", {
  ...
  DeleteUser: async (userid) => {
    ...
    try {
      ...
      const res = await axiosInstanceAuthService.delete(
      ...
      if (res?.status === 200 && res?.data?.status === true) {
        set((state) => ({
          allUsers: state.allUsers.filter(user.id !== userid),
        }));
