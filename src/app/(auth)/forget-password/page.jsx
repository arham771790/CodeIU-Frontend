...
export default function ForgetPassword() {
    ...
    useEffect(() => {
        const verifyId = async () => {
            ...
            try {
                ...
                const res = await axiosInstanceAuthService.get(`/auth/verify-id/${verificationId}`);

                console.log(res);
                
                if (res?.data?.success || res?.data === true || res?.data?.status === true) {
                    setIsVerified(true);
                } else {
                    setIsVerified(false);
    ...
    const handleSubmit = async (e) => {
        ...
        if (validateForm()) {
            ...
            const payload = {
                ...
                confirmPassword: formData.confirmPassword, // Included just in case backend needs it
                id: verificationId,
                // Any other params?
            };
    ...
    if (!isVerified) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-black font-sans">
                <div className="bg-gray-100 p-8 rounded-2xl shadow-2xl max-w-md w-full text-center">
                    ...
                    <Link href="/login" className="px-6 py-3 bg-black text-white font-bold rounded-lg hover:bg-gray-800 transition-colors">
                        Go to Login
                    </Link>
                </div>
            </div>
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-black font-sans">
            <div className="relative flex flex-col w-full max-w-md m-4 bg-gray-100 shadow-2xl rounded-2xl overflow-hidden p-8 sm:p-12">
                <h3 className="text-xl font-semibold text-gray-500 mb-2">Security</h3>
                <h1 className="text-3xl font-bold text-black mb-1">Reset Password</h1>
                <p className="text-gray-600 mb-8">Create a new strong password.</p>
