...
export const useAuthStore = create((set, get) => ({
  ...
  // Added checkAuthPromise caching to deduplicate requests during the same render cycle
  checkAuthPromise: null,

  checkAuth: async () => {
    const { checkAuthPromise } = get();
    ...
    const promise = (async () => {
      try {
        ...
        set({ isCheckingAuth: false, checkAuthPromise: null }); // Reset promise reference
      }
    })();
  ...
  updateProfile: async (payload) => {
    try {
      const res = await axiosInstanceAuthService.put("/auth/update-profile", payload, {
        withCredentials: true,
      });
      if (res?.data?.success) {
